...
matching_lib = ctypes.CDLL('./matching.dll')
matching_lib.find_nearest_indices.argtypes = [ctypes.POINTER(ctypes.c_double), ctypes.c_int,
                                              ctypes.POINTER(ctypes.c_double), ctypes.c_int,
                                              ctypes.POINTER(ctypes.c_int)]
matching_lib.find_nearest_indices.restype = None
...
def match_timestamps(timestamps1: np.ndarray, timestamps2: np.ndarray) -> np.ndarray:
    ...
    Example:
        ...
        => matching = [0, 1, 3]
    """
    timestamps1_ct = timestamps1.astype(np.float64)
    timestamps2_ct = timestamps2.astype(np.float64)
    matching = np.zeros(timestamps1.shape[0], dtype=np.int32)
    matching_lib.find_nearest_indices(timestamps1_ct.ctypes.data_as(ctypes.POINTER(ctypes.c_double)),
                                      timestamps1_ct.size,
                                      timestamps2_ct.ctypes.data_as(ctypes.POINTER(ctypes.c_double)),
                                      timestamps2_ct.size,
                                      matching.ctypes.data_as(ctypes.POINTER(ctypes.c_int)))
    return matching
